...
class BUFFALO_MOVE_TO_BUFFALO(Behaves):
    ...
    def act(self, world: World):
        target = world.findnearesttarget(
            entity=self.actor, targetlist=type(self.actor), findrange=self.actor.sight
        )
...
class CHARGE_RUSH(Behaves):
    ...
    def act(self, world: World):
        ...
        predators = [
            an for an in nearby_animals if an.__class__.__name__ in predator_names
        ]
...
class ATTACK_TARGET(Behaves):
    ...
    def act(self, world: World):
        if self.predator.hunger > self.predator.MAXHUNGER * 0.8:
            return False
        target = world.findnearesttarget(
            self.predator, self.target_types, findrange=self.predator.sight
        )
        if target and isinstance(target, Animal):
            ...
            if self.predator.pos.distance_to(target.pos) > 50:
                self.predator.move(
                    self.predator.speed * 1.3,
                    self.predator.pos
                    + normalize(
                        target.pos + normalize(Vector2(0, 0)) * 30 - self.predator.pos
                    )
                    * self.predator.speed
                    * 1.3,
                )
...
class EAT_GRASS(Behaves):
    ...
    def act(self, world: World):
        ...
        if (
            world.grass_map[grassy][grassx].remain <= self.eat + 0.1
            or self.eater.hunger > self.eater.MAXHUNGER * 0.9
        ):
            return False
        ...
        if target:
            ...
            if self.actor.pos.distance_to(target.pos) > 30:
                self.actor.move(
                    self.actor.speed,
                    self.actor.pos
                    + normalize(target.pos - self.actor.pos) * self.actor.speed,
                )
            else:
                target.remain -= 0.5
...
class LION_GUARD_CARCASS(Behaves):
    ...
    def act(self, world: World) -> bool:
        ...
        hyena = world.findnearesttarget(
            self.actor, (Hyena,), findrange=self.actor.sight
        )
        if hyena is None:
            return False
    ...
    def act(self, world: World) -> bool:
        ...
        if len(nearby_lions) + 1 >= self.MIN_LIONS - 1:
            for lion in nearby_lions:
                lion.move(
                    lion.speed, lion.pos + normalize(target.pos - lion.pos) * lion.speed
                )
...
#AI 활용
    ...
    def act(self, world: World) -> bool:
        ...
        if elephant is not None:
            if elephant.pos.distance_to(self.actor.pos) < 40:
                return False
            self.actor.move(
                self.actor.speed * 1.3,
                self.actor.pos
                + normalize(elephant.pos - self.actor.pos) * self.actor.speed * 1.3,
            )
        else:
            ...
            away = (
                self.actor.pos
                + normalize(self.actor.pos - predator.pos) * self.actor.speed * 1.2
            )
...
class WANDER(Behaves):
    ...
    def act(self, world: World):
        ...
        if self.actor.dir.length_squared() == 0:
            self.actor.dir = Vector2(1, 0).rotate(r.uniform(0, 360))
        ...
        new_dir = Vector2(1, 0).rotate(angle + delta)

        # 기존 방향에 높은 가중치
        self.actor.dir = self.actor.dir * 0.6 + new_dir * 0.4
        ...
        # 이동 목표
        target = self.actor.pos + self.actor.dir * self.actor.speed
        ...
        # 아주 가끔 크게 방향 변경
        if r.random() < 0.03:
            self.actor.dir.rotate_ip(r.uniform(-120, 120))

        self.actor.move(
            self.actor.speed, self.actor.pos + self.actor.dir * self.actor.speed
        )
...
class ATTACK_LION_WHEN_MANY(Behaves):
    ...
    def act(self, world: World):
        ...
        lion_target = world.findnearesttarget(
            self.actor, Lion, findrange=self.actor.sight
        )
        ...
        if len(nearby_hyena) + 1 >= 6:
            ...
            if self.actor.pos.distance_to(lion_target.pos) > 30:
                self.actor.move(
                    self.actor.speed * 1.3,
                    self.actor.pos
                    + normalize(lion_target.pos - self.actor.pos) * self.actor.speed,
                )
            else:
