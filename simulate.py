...
class Simulator:
    ...
    def handle_user_input(self):
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                cansummon = (
                    0 < mousepos.x < self.SCREEN_H and 0 < mousepos.y < self.SCREEN_W
                )
    ...
    def draw_background(self):
        ...
        pygame.draw.rect(
            self.screen, (200, 200, 255), (-2, -2, self.SCREEN_H + 4, self.SCREEN_W + 4)
        )
        self.screen.blit(
    ...
    def draw_entities(self):
        ...
        for e, loc in sorted(
            self.world.entity_map.items(), key=lambda item: item[0].layer
        ):
            surface = e.surface()
            if surface is None:
                continue
            rect = (
                surface.get_rect(center=loc - pygame.Vector2(0, e.radius * 0.3))
                if isinstance(e, Explosion)
                else surface.get_rect(center=loc)
            )
    ...
    def draw_bar(self, entity, pos, surface):
        ...
        pygame.draw.rect(self.game_surface, (40, 40, 40), (x, y, width, height))
        ...
        pygame.draw.rect(
            ...
            (
                (255, 255, 0)
                if hpratio > 0.6
                else (255, 165, 0) if hpratio > 0.3 else (255, 99, 71)
            ),
            (x, y, width * hpratio, height),
        )

        # 테두리
        pygame.draw.rect(self.game_surface, (255, 255, 255), (x, y, width, height), 1)

        pygame.draw.rect(self.game_surface, (40, 40, 40), (x, y + 8, width, height))

        # 배고픔
        pygame.draw.rect(
            self.game_surface, (160, 110, 60), (x, y + 8, width * hungerratio, height)
        )

        # 테두리
        pygame.draw.rect(
            self.game_surface, (255, 255, 255), (x, y + 8, width, height), 1
        )
    ...
    def draw_UI(self):
        self.draw_text(
            self.screen,
            f"[{self.world.worldtime//3600:02}:{self.world.worldtime%3600//60:02}] {self.xspeed:.2f}배속"
            + ("(일시정지됨)" if self.pause else ""),
            10,
            self.fullSCREEN_W - 50,
            30,
        )
        if self.grassmap:
        ...
        self.draw_text(
            self.screen,
            "[esc] 종료 [space] 일시정지 [z] NUCLEAR [x] 로그 지우기 [c] 풀 보이기",
            10,
            755,
            20,
        )
        self.draw_text(
            self.screen,
            "[1] Elephant [2] Lion [3] Hyena [4] Buffalo [5] Zebra [6] Gazelle [7] Carcass",
            10,
            785,
            20,
        )
        self.draw_text(
            self.screen,
            "[↑] +0.1배속 [↓] -0.1배속 [→] +1배속 [←] -1배속 [마우스 휠] ±0.5배속씩 조정",
            10,
            815,
            20,
        )
        if self.hover is not None:
            self.draw_text(
                self.screen,
                f"이름 {self.hover.name} 종 {self.hover.__class__.__name__.upper()}",
                750,
                755,
                20,
            )
            self.draw_text(
                self.screen,
                f"체력 {self.hover.hp:.0f}/{self.hover.maxhp:.0f} 배고픔 {self.hover.hunger:.0f}/{self.hover.MAXHUNGER:.0f}",
                750,
                785,
                20,
            )
            self.draw_text(
                self.screen,
                f"속도 {self.hover.speed:.1f} 공격력 {self.hover.attack} 방어력{self.hover.defense} 시야 {self.hover.sight}",
                750,
                815,
                20,
            )
    ...
    def draw_log(self):
        ...
        # 배경
        pygame.draw.rect(self.screen, (25, 25, 25), (log_x, log_y, log_w, log_h))

        pygame.draw.rect(self.screen, (200, 200, 255), (log_x, log_y, log_w, log_h), 2)
        ...
        for i, line in enumerate(lines):
            ...
            self.screen.blit(surface, (log_x + 5, log_y + i * line_height))
        # 로그 글씨쓰기 AI도움

    def update_grassmap(self):
        self.grass_surface = pygame.Surface((1200, 750), pygame.SRCALPHA)
        ...
        for y in range(10):
            for x in range(16):
                ...
                ratio = max(0, min(value / 20, 1))

                # 빨강 -> 노랑 -> 초록
                color = (int(255 * (1 - ratio)), int(255 * ratio), 0, 100)

                rect = pygame.Rect(x * 75, y * 75, 75, 75)

                pygame.draw.rect(self.grass_surface, color, rect)

                pygame.draw.rect(self.grass_surface, (50, 50, 50), rect, 1)

                text = self.font.render(f"{value:.2f}", True, (255, 255, 255))

                text_rect = text.get_rect(center=rect.center)

                self.grass_surface.blit(text, text_rect)

    def get_hovered_animal(self):
        mouse_pos = pygame.mouse.get_pos()

        for entity, pos in reversed(
            sorted(self.world.entity_map.items(), key=lambda item: item[0].layer)
        ):
